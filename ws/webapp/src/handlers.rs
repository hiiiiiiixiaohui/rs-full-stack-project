...
pub async fn get_all_teachers(tmpl: web::Data<tera::Tera>) -> Result<HttpResponse, Error> {
    ...
    let res = awc_client
        .get("http://localhost:3030/teachers/")
        .send()
        .await
        .unwrap()
    ...
    let s = tmpl.render("teachers.html", &ctx).map_err(SysError::from)?;
...
pub async fn show_register_form(tmpl: web::Data<tera::Tera>) -> Result<HttpResponse, Error> {
    ...
    let s = tmpl.render("register.html", &ctx).map_err(SysError::from)?;
...
pub async fn handle_register(
    ...
    if params.name == "Dave" {
        ...
        s = tmpl.render("register.html", &ctx).map_err(SysError::from)?;
    } else {
        ...
        let res = awc_client
            .post("http://localhost:3030/teachers/")
