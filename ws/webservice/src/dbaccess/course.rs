...
pub async fn get_course_details_db(
    ...
    if let Some(course) = row {
        ...
        Err(SysError::NotFound("Course Id not found".into()))
    }
}

pub async fn post_new_course_db(pool: &PgPool, new_course: CreateCourse) -> Result<Course, SysError> {
    let row = sqlx::query_as!(
        Course,
        r#"INSERT INTO course (teacher_id, name, description, format, structure, price, duration, language, level)
        VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
        RETURNING id, teacher_id, time, name, description, format, structure, price, duration, language, level"#,
...
pub async fn delete_course_db(pool:&PgPool, teacher_id:i32, id:i32) -> Result<String,SysError> {
...
pub async fn update_course_datails_db(
    pool:&PgPool,
    teacher_id: i32,
    id:i32,
    ...
    let current_course_row = sqlx::query_as!(
        ...
        teacher_id,
        id
    ).fetch_one(pool).await.map_err(|_err|SysError::NotFound("Course Id Not Found".into()))?;
