...
pub async fn get_all_teachers_db(pool: &PgPool) -> Result<Vec<Teacher>, SysError> {
    ...
    match teachers.len() {
        0 => Err(SysError::NotFound("No teachers found".into())),
        _ => Ok(teachers),
    }
}

pub async fn get_teacher_details_db(pool: &PgPool, teacher_id: i32) -> Result<Teacher, SysError> {
    let row = sqlx::query!(
    ...
    .map_err(|_err| SysError::NotFound("Teacher Id not found".into()))?;
...
pub async fn delete_teacher_db(pool: &PgPool, teacher_id: i32) -> Result<String, SysError> {
    let row = sqlx::query(&format!("DELETE FROM teacher where id = {}", teacher_id))
        .execute(pool)
        .await
        .map_err(|_err| SysError::NotFound("Unable to delete teacher".into()))?;
