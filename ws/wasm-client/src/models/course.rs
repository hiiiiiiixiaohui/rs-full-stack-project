use super::super::errors::SysError;
...
pub async fn get_courses_by_teacher(teacher_id: i32) -> Result<Vec<Course>, SysError> {
    let mut opts = RequestInit::new();
    opts.method("GET");
    opts.mode(RequestMode::Cors);

    let url = format!("http://localhost:3030/courses/{}", teacher_id);
    ...
    let courses: Vec<Course> = from_value(json).unwrap();
...
pub async fn delete_course(teacher_id: i32, course_id: i32) {
    ...
    let url = format!("http://localhost:3030/courses/{}/{}", teacher_id, course_id);
    ...
    let _course: Course = from_value(json).unwrap();
}
...
pub async fn add_course(name: String, description: String) -> Result<Promise, JsValue> {
    ...
    let url = "http://localhost:3030/courses/";
